# ARM 安卓平台端侧多模态大模型部署调研报告

> **摘要：**本报告基于 RK3588S 硬件平台完成端侧离线多模态大模型全量实测：
...
在 ARM 安卓设备上部署端侧多模态大语言模型，实现图片理解、视觉问答、多轮对话等 AI 交互能力。
...
> 本节流程对应的可执行脚本已开源，见 [`llama.cpp_and_GGUF/`](llama.cpp_and_GGUF/)：模型下载、adb 推送、Termux 编译、功能与性能测试、日志解析五个阶段，含调用参数与已知问题排查。
...
两条推理引擎路线均可调度不同的硬件后端。以下是各硬件后端的可行性评估：
...
- 测试环境：RK3588S 12GB，Android 12，2026‑08‑11
